...
With no analytical solution available for the governing equation PDE system numerical approaches need to be applied. As already mentioned \texttt{ANSYS FLUENT} which is a VOF solver is used to build the model. The method used to calculate the properties for each cell is called Finite Volume Method.
...
The Schmidt number is only influenced by thermophysical constants. The already mentioned diffusion constant $D$ and the viscosity $\nu$ affect this variable. Both are adapted when changing $Sc$ during the parametric study.
