...
# Set dependency on FLUENT calculation (currently broken cause job-id is not accessable here)
...
python -m pip install -r ./hpc_requirements.txt >> ${LOG_FILE} 2>&1
python ./transient_field.py >> $LOG_FILE 2>&1
